...
|----------|-----------------|---------------------|-----------------------------------------------------|
| **CH1**  | Right Stick (X) | **Body Roll**       | Additive lean left/right (Active in all modes)      |
| **CH2**  | Right Stick (Y) | **Body Pitch**      | Additive lean forward/back (Active in all modes)    |
...
| **CH6**  | Switch SD       | **Light Control**   | High: Off / Low: On                                 |
| **CH7**  | Switch SB       | **Standing Height** | 3-Pos: -1:Spider (Low) / 0:Standard / 1:Tall        |
| **CH8**  | Switch SC       | **Transmission**    | 3-Pos: -1:Forward / 0:Neutral (Manual) / 1:Backward |
| **CH9**  | Button SE       | **Kill-Switch**     | Immediate Hard cutoff                               |
...
* **Armed State (High):** Activates the Inverse Kinematics (IK) engine. The robot transitions from a "seated" position to the operational height defined by Switch SB.
* **Disarm State (Low):** Triggers the **Soft Cutoff**. The system remains active until the robot is safely grounded to prevent mechanical shock to the geartrain.

#### 2. Individual Leg Manual Override (CH11 - CH14)
* **Activation:** This mode is active only when **Switch SC** is in the **Neutral (0)** position and **resets** when switching the transmission.
* **Leg Selection (CH14):** Using the Left X-Trim, the operator cycles through the 6 legs. 
* **Joint Control:** Trims CH11, CH12, and CH13 allow for precise, non-IK positioning of the Coxa, Femur, and Tibia joints. This is designed for clearing mechanical obstructions or maintenance.

#### 3. Immediate Hard Cutoff (Kill-Switch - Button SE)
* **Emergency Override:** A high-priority software interrupt. 
* **Execution:** Immediately halts all PWM generation and puts the controller into deep sleep. Must be held down for 3 seconds.
* **Context:** Used for critical scenarios such as thermal runaway, mechanical obstruction, or kinematic singularities where instant de-energization is required.

#### 4. Soft Cutoof
* **Transition:** The ESP32 gradually reduces the Z-axis altitude of the chassis. Once the chassis reaches the minimum ground clearance (0mm), the PWM signals to the PCA9685 are terminated, and the controller put into deep sleep.
* **Purpose:** Protects the MG995 metal gears from gravity-induced impact and structural fatigue, ensuring long-term actuator reliability.
...
- **Logic:** 5.2V for ESP32, Servoshields, and ELRS Receiver
...
- **Singularity Protection:** Mathematical limits in the Inverse Kinematics (IK) code must prevent the Coxa, Tibia and Femur from reaching collinearity, which could cause mechanical locking or servo over-torque.
